...
__all__ = ['login', 'logout', 'verify_token']
...
def login(request, next_page=None, required=False, gateway=False):
    ...
    if ticket:
        ...
        if user is not None:
            # don't allow non-netid users to come through here, nor non-validated email addresses.
            if not user.is_netid:
                return HttpResponse('That NetID is not valid.', status=409)
            elif not user.email_verified:
                return HttpResponse('You must verify your email address.', status=409)
            ...
            # TODO redirect user to /#/register using a reverse('apps.core.views.home') + '#/register'
            return HttpResponseRedirect(reverse('apps.core.views.home') + '#/register')
    else:
        if gateway:
            return HttpResponseRedirect(_login_url(service, ticket, True))
        else:
...
def logout(request, next_page=None):
    ...
    if hasattr(request.user, 'is_netid') and request.user.is_netid:
        if not next_page:
            next_page = _redirect_url(request)
